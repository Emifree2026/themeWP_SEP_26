...
<div class="pd-root min-h-screen bg-white">
	...
	<div class="bg-slate-50 border-b border-slate-200">
		<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 pb-8">
			<a href="<?php echo esc_url( home_url( '/en/' ) ); ?>" class="inline-flex items-center gap-2 text-blue-700 hover:text-blue-800 font-medium mb-3 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 rounded">
	...
	<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
		<div data-pd-tab="quick" class="pd-tab-panel">
			<div class="bg-white border border-slate-200 rounded-2xl p-6">
				<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
					...
					<button type="button" data-pd-quick-calc class="self-start sm:self-auto inline-flex items-center gap-2 px-4 py-2 bg-blue-700 hover:bg-blue-800 text-white rounded-lg font-medium text-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500" data-pd-i18n="calculate">Calculate</button>
				</div>
		...
		<?php /* Guided tour video. Lite-embed pattern: thumbnail + click-to-play
		   so YouTube's heavy iframe (HTML, CSS, ~700KB of JS, tracking pixels,
		   cookies) doesn't load until the user actively chooses to watch.
		   - <img loading="lazy"> defers the thumbnail fetch until the card is
		     near the viewport.
		   - maxresdefault.jpg (1280x720) is preferred with onerror fallback
		     to hqdefault.jpg (480x360, always exists for every YouTube video).
		   - The <a> href points at the YouTube watch URL so no-JS users
		     (and right-click "open in new tab") still get the video.
		   - Click handler swaps the <a> for an <iframe> with autoplay=1. */ ?>
		<section class="mt-10" aria-labelledby="pd-video-en">
			<div class="bg-white border border-slate-200 rounded-2xl p-6">
				<div class="flex items-center gap-3 mb-4">
					...
					<div>
						<h2 id="pd-video-en" class="text-lg font-bold text-zinc-900 leading-tight">Watch the guided tour</h2>
						<p class="text-sm text-slate-600">Optional 3-minute walkthrough of the calculator and its inputs.</p>
					</div>
				</div>
				...
				<div class="relative w-full overflow-hidden rounded-xl border border-slate-200 bg-slate-900" style="padding-top: 56.25%;">
					<a href="https://www.youtube.com/watch?v=NJvomqKi0ZI"
					   ...
					   data-yt-title="Air Pressure Loss Calculator walkthrough"
					   aria-label="Play Air Pressure Loss Calculator walkthrough (YouTube)">
						<img
		...
		<script>
		// Lite-embed click-to-play. Until the user clicks, no YouTube
		// JS / cookies / tracking pixels are loaded — the only network
		// hit is the lazy <img> thumbnail. On click, swap <a> for an
		...
		document.querySelectorAll( '.emifree-yt-lite' ).forEach( function ( lite ) {
			lite.addEventListener( 'click', function ( e ) {
				// Allow modifier-clicks (cmd/ctrl/shift/middle) to open
				// the YouTube watch URL in a new tab — that's the
				// standard browser UX for media embeds.
